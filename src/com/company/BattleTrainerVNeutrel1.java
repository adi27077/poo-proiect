...
public class BattleTrainerVNeutrel1 implements Battle {
    ...
    public BattleTrainerVNeutrel1(String trainer1, String trainer2, Pokemon pokemon1,
    ...
    public void execute() {
        ...
        bl.log("========Tip Batalie: Trainer vs Neutrel1========");
        bl.log(pokemon1.getName() + " (" + trainer1 + ") vs Neutrel1");
        ...
        Pokemon neutrel1 = Game.getInstance().getNeutrel1();
        int hpNeutrel11 = neutrel1.getHp();
        int hpNeutrel12 = neutrel1.getHp();

        Random r = new Random();

        bl.log(pokemon1.toString());
        bl.log(neutrel1.toString());
        ...
        while(hp1 > 0 && hpNeutrel11 > 0) {
            ...
            switch (atkType) {
                case 0 -> {
                    if(pokemon1.getNormalAtk() != 0) {
                        dmg1to2 = pokemon1.getNormalAtk() - neutrel1.getDef();
                        atk1 = pokemon1.getName() + " atac normal";
                    } else {
                        dmg1to2 = pokemon1.getSpecialAtk() - neutrel1.getSpecialDef();
            ...
            if(!p2Stunned) {
                dmg2to1 = neutrel1.getNormalAtk() - pokemon1.getDef();
                atk2 = "Neutrel1 atac normal";
            } else {
                dmg2to1 = 0;
                atk2 = "Neutrel1 nu face nimic";
            }
            if(p1Dodge)
                dmg2to1 = 0;

            hp1 -= Math.max(dmg2to1, 0);
            hpNeutrel11 -= Math.max(dmg1to2, 0);
            ...
            String logText = atk1 + " / " + atk2 + "\n" + pokemon1.getName() + " HP: " + hp1 +
                     ...
                     "\nNeutrel1 HP: " + Math.max(hpNeutrel11, 0) +
        ...
        if(hp1 > 0) {
            ...
            bl.log(trainer1 + " (" + pokemon1.getName() + ") pierde lupta contra Neutrel1");
        }

        bl.log(pokemon2.getName() + " (" + trainer2 + ") vs Neutrel1");
        if(item2 != null) {
        ...
        while(hp2 > 0 && hpNeutrel12 > 0) {
            ...
            switch (atkType) {
                case 0 -> {
                    if(pokemon2.getNormalAtk() != 0) {
                        dmg1to2 = pokemon2.getNormalAtk() - neutrel1.getDef();
                        atk1 = pokemon2.getName() + " atac normal";
                    } else {
                        dmg1to2 = pokemon2.getSpecialAtk() - neutrel1.getSpecialDef();
            ...
            if(!p2Stunned) {
                dmg2to1 = neutrel1.getNormalAtk() - pokemon2.getDef();
                atk2 = "Neutrel1 atac normal";
            } else {
                dmg2to1 = 0;
                atk2 = "Neutrel1 nu face nimic";
            ...
            hpNeutrel12 -= Math.max(dmg1to2, 0);

            String logText = atk1 + " / " + atk2 + "\n" + pokemon2.getName() + " HP: " + hp2 +
                    ((p1Dodge) ? " (dodge) " : " ") +
                    ((p1Ability1cd != 0) ? (", Abilitate 1 cooldown " + p1Ability1cd + " ") : " ") +
                    ((p1Ability2cd != 0) ? (", Abilitate 2 cooldown " + p1Ability2cd + " ") : " ") +
                    "\nNeutrel1 HP: " + Math.max(hpNeutrel12, 0) +
                    ((p2StunnedNextRound) ? " (stunned)\n" : "\n");
        ...
        if(hp2 > 0) {
            ...
            bl.log(trainer2 + " (" + pokemon2.getName() + ") pierde lupta contra Neutrel1");
        }
